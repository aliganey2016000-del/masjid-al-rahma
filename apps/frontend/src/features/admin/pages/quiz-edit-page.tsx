 ...
 * takes over the entire workspace (no chapter/lesson/assignment list), with
...
export function QuizEditPage() {
  ...
  // Locate the chapter + item indices for this quiz.
  let chapterIdx = -1;
  let itemIdx = -1;
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--color-surface-secondary)]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-[var(--color-text-tertiary)]">Loading quiz...</p>
        </div>
      </div>
    );
  }
  ...
  // -----------------------------------------------------------------------
  // Quiz not found (bad id, deleted, or not a quiz item)
  // -----------------------------------------------------------------------
  if (!quiz) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--color-surface-secondary)]">
        <div className="text-center space-y-4">
          ...
          <p className="font-semibold text-[var(--color-text-primary)]">Quiz not found</p>
          <button
  ...
  // -----------------------------------------------------------------------
  // Main — full-page editor, no chapter/lesson/assignment list
  // -----------------------------------------------------------------------
  return (
        ...
        <div className="mx-auto max-w-3xl px-4 py-3 flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <h1 className="text-sm lg:text-base font-bold text-[var(--color-text-primary)] truncate flex items-center gap-1.5 mt-0.5">
              <span>❓</span> {content?.chapters[chapterIdx]?.title} · Quiz {itemIdx + 1}
            </h1>
          </div>
