...
// Content is "markdown-shaped" if it has markdown markers (headers, bold,
// bullets) but no HTML tags — i.e. it was authored as Markdown (e.g. pasted
// from an AI chat) rather than produced by this editor's own HTML output.
function looksLikeMarkdown(text: string): boolean {
  if (!text.trim()) return false;
  if (/<\/?[a-z][\s\S]*>/i.test(text)) return false;
  return /^#{1,6}\s/m.test(text) || /\*\*[^*]+\*\*/.test(text) || /^[-*]\s/m.test(text) || /^>\s/m.test(text);
}
...
export function RichTextEditor({ value, onChange, placeholder }: RichTextEditorProps) {
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] overflow-hidden">
      <div className="flex items-center justify-between gap-2 bg-[var(--color-surface-tertiary)] px-2 py-1">
        ...
        {tab === 'code' && looksLikeMarkdown(value) && (
          <button
            type="button"
            onClick={() => { onChange(markdownToHtml(value)); setTab('visual'); }}
            className="rounded-md bg-primary-600 px-2.5 py-1 text-xs font-semibold text-white hover:bg-primary-700 transition-colors whitespace-nowrap"
            title="This looks like Markdown, not HTML — convert it so Visual/Student View render it formatted"
          >
      ...
      {tab === 'preview' ? (
        ...
        <textarea
          ...
          onChange={(e) => onChange(e.target.value)}
          onPaste={(e) => {
            // Pasting a whole Markdown document into an empty field is the
            // common case (e.g. copying lesson content generated by an AI
            // chat) — auto-convert so Visual/Student View render it
            // formatted instead of as a raw text wall. Leave non-empty
            // fields alone (use the Convert button) to avoid clobbering
            // in-progress edits.
            if (value.trim()) return;
            const pasted = e.clipboardData.getData('text/plain');
            if (!looksLikeMarkdown(pasted)) return;
            e.preventDefault();
            onChange(markdownToHtml(pasted));
          }}
