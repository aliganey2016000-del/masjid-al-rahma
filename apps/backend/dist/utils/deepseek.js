...
exports.generateLessonHtml = generateLessonHtml;
...
async function generateLessonHtml(sourceText) {
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey) {
        throw new api_error_1.InternalServerError('AI lesson generation is not configured on this server (missing DEEPSEEK_API_KEY).');
    }
    const trimmed = (sourceText || '').trim();
    if (!trimmed) {
        throw new api_error_1.BadRequestError('No source text to generate a lesson from.');
    }
    ...
    let response;
    try {
        response = await axios_1.default.post(DEEPSEEK_API_URL, {
            model: 'deepseek-chat',
            messages: [
        ...
        throw new api_error_1.InternalServerError(`DeepSeek request failed${status ? ` (${status})` : ''}: ${detail}`);
    }
    const raw = response.data?.choices?.[0]?.message?.content || '';
    const html = stripCodeFences(raw);
    if (!html) {
        throw new api_error_1.InternalServerError('DeepSeek returned an empty response. Please try again.');
    }
    return html;
}
...
`.trim();
async function generateQuizQuestions(sourceText, customInstructions, counts) {
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey) {
        throw new api_error_1.InternalServerError('AI quiz generation is not configured on this server (missing DEEPSEEK_API_KEY).');
    }
    const trimmed = (sourceText || '').trim();
    if (!trimmed) {
        throw new api_error_1.BadRequestError('No source material to generate quiz questions from.');
    }
    const validCounts = (counts || []).filter((c) => c && c.type && c.count > 0);
    if (validCounts.length === 0) {
        throw new api_error_1.BadRequestError('Select at least one question type and count.');
    }
    const clipped = trimmed.length > MAX_SOURCE_CHARS
        ? `${trimmed.slice(0, MAX_SOURCE_CHARS)}\n\n[...source truncated for length...]`
        : trimmed;
...
- Vary the specific facts/angles tested across questions of the same type so they don't repeat each other.${customInstructions?.trim() ? `\n- Additional instructions from the teacher: ${customInstructions.trim()}` : ''}`;
    let response;
    try {
        response = await axios_1.default.post(DEEPSEEK_API_URL, {
            model: 'deepseek-chat',
    ...
    try {
        ...
        throw new api_error_1.InternalServerError('DeepSeek returned malformed JSON. Please try again.');
    }
    const questions = Array.isArray(parsed?.questions) ? parsed.questions : [];
    if (questions.length === 0) {
        throw new api_error_1.InternalServerError('DeepSeek did not return any questions. Please try again.');
    }
    return questions.map((q) => normalizeAiQuestion(q));
}
// ---------------------------------------------------------------------------
        ...
        case 'swipe_sort': {
            ...
            if (cards.length < 2) {
                while (cards.length < 2)
                    cards.push({ text: '', correctSide: 'left' });
            }
            return {
                ...base,
